...
pub fn day5_main(input: i32) -> Result<(), Box<dyn std::error::Error>> {
...
impl Intputer {
    ...
    fn execute(&mut self) {
        ...
        match addressing::Instruction::from(current_instruction) {
            ...
            addressing::Instruction::CompareLessThan(source1, source2) => {
                let source1 = self.get_parameter(source1, 1);
                let source2 = self.get_parameter(source2, 2);
                let dest = self.get_parameter(addressing::AddressingMode::Immediate, 3);

                self.program[dest as usize] = if source1 < source2 {
                    1
                } else {
                    0
                };
            ...
            addressing::Instruction::CompareEquals(source1, source2) => {
                let source1 = self.get_parameter(source1, 1);
                let source2 = self.get_parameter(source2, 2);
                let dest = self.get_parameter(addressing::AddressingMode::Immediate, 3);
